...
export function renderScope(c, payload, meta) {
  ...
  /* 安全线 30 天是**这张图的显示参照线**，不是模块参数。
     模块的门槛参数里没有「安全线天数」这一档（有的是安全库存天数、目标覆盖天数
     120、超量判定消化天数 365，量级都不同），所以不假装它来自参数 ——
     参考图用的就是 30 天，这里跟着它，并在 ⓘ 里写明是显示阈值。 */
  const safetyDays = 30;
  ...
  h += sec('库存结构与覆盖' + aiMarkHTML('库存总体分析'),
    chip('derived') + info(`<b>口径</b><br>${s.cover_days_note}<br><br>
      ...
      一个 900 天的滞销子体会把整款拉高。低于安全线 ${safetyDays} 天的染警示色。<br>
      这条 ${safetyDays} 天的安全线是<b>本图的显示参照线</b>，不是模块的门槛参数
      —— 模块参数里没有这一档（有的是安全库存天数、目标覆盖天数、超量判定消化天数，
      量级都不同）。要把它变成可调门槛，得先在 Agent 配置页立一个参数。`),
