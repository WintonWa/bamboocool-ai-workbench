 ...
 * 逐日销量与需求柱状图
 *
 * 设计规格：01-方案与数据需求/06-界面优化点登记.md R2
 *
 * 按数据性质分四种画法，一根日期轴贯穿历史与未来：
 *   量           柱      实际销量 / 预测销量        yAxis 0
 *   有起止的状态 横条    BD / Coupon / 广告 / 价格   yAxis 1（固定量程的隐藏泳道轴）
 *   点事件       散点    单日事件                    yAxis 1
 *   连续指标     折线    价格 / 金额 / 流量          yAxis 2、3
 *   柱不可信     改柱样式 缺货日、异常日（不进泳道）
 *
 * 颜色分工：量用中性墨色，彩色只留给事件类型 —— 区分类别才是彩色该干的活。
 * ------------------------------------------------------------------------- */
...
/* 主入口：把 chart payload 画成 ECharts 配置并挂上交互。
   ...
   后面的渲染出现，看起来像"闪一下才有图"。 */
export async function renderDayChart(el, a) {
  const ch = a.chart;
  disposeChart();
  if (!ch.available) {
  ...
  if (ch.has_forecast) {
    series.push({
      ...
      itemStyle: { color: CH.forecastBar },
