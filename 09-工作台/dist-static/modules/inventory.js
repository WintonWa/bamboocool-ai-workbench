...
import { disposeDayChart } from '../daychart.js';
import { renderDetail, setPageCtx } from './inventory/page-detail.js';
...
export async function render(ctx) {
  ...
  if (pageId === 'inv-risk') {
    // 传整个载荷：两张交叉图要逐行数 rows，overview 里只有边缘分布
    renderRisk(host.body, payload, meta);
    // 高风险清单点行进单品盘点。委托挂在 body 上 —— 这段每次切页都重建
    if (!host.body.dataset.riskBound) {
      host.body.dataset.riskBound = '1';
      host.body.addEventListener('click', (e) => {
        const tr = e.target.closest ? e.target.closest('tr[data-open]') : null;
        if (tr) ctx.open(tr.getAttribute('data-open'), 'inv-detail');
      });
    }
    return;
  }
  if (pageId === 'inv-scope') {
    // 传整个载荷：款式覆盖天数与影响件数要逐行汇总 rows，scope 里只有合计
    renderScope(host.body, payload, meta);
    return;
  }
  ...
  setPageCtx(ctx);          // 方案下拉要 ctx.setParams
