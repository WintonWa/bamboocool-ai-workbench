...
/* 板块 → 背后的 Agent。「这一屏立在什么数据上」是数据自述，不是判断，不标。 */
const OVW_AGENT = {
  '今天该看什么': '运营总览',
  '四个模块当前的判断': '运营总览',
};
...
function sectionHead(ctx, title, tail) {
  const mark = ctx.aiMark(OVW_AGENT[title]);
  const pick = mark ? ctx.schemePicker(OVW_AGENT[title]) : null;
  const h = ctx.el('div', { class: 'ovw-sec-head' },
    [ctx.el('span', { class: 'ovw-sec-title' }, [title, mark, pick])]);
...
function renderHeadline(ctx, d) {
  ...
  /* 板块标题留在卡片**外面**、和其余四个板块同一档（--fs-title 24 + 分隔线）。
     灵感稿把它做成卡片内的小字，这里刻意不照抄：一页上五个板块标题必须一样大，
     只有这一个小一档会读成「它是别人的子标题」，页面层级就断了。
     一致性压过复刻 —— 要改就五个板块一起改，不是单改这一个。 */
  sec.appendChild(
  ...
  /* 三格并排。格内构图照灵感稿 Image2-视觉灵感-2026-09-04/images/01-运营总览.png：
     圆底序号 + 标题 + 徽标 / 一句话 / 去处理 ›，格与格之间一道竖线。
     格数跟着 lines 走（少一条就是两格），栅格用 auto-fit 不写死 3。 */
  const go = (d.headline && d.headline.go_label) || '';
  const list = ctx.el('div', { class: 'ovw-lede-list' });
  (d.headline.lines || []).forEach((ln, i) => {
    const col = ctx.el('div', { class: 'ovw-lede-col' });
    col.appendChild(ctx.el('div', { class: 'ovw-lede-head' }, [
      ctx.el('span', { class: 'ovw-lede-no', text: String(i + 1) }),
      ctx.el('span', { class: 'ovw-lede-title', text: ln.title || ln.module_label || '' }),
      /* 徽标的词和色调都由后端给（COPY 的 badge_* + 五个语义色之一），
         前端不判词也不挑色 —— 判了就成了总览自己的第二套分级。 */
      ln.badge
        ? ctx.el('span', {
            class: 'ovw-lede-badge',
            'data-tone': ln.badge.tone || 'mid',
            text: ln.badge.label || '',
          })
        : null,
    ]));
    /* 句子和跳转在稿子里是缩进到标题左边缘的（不贴圆序号），所以它们套一层
       带左内缩的容器，缩进量 = 圆序号直径 + 那个间距，见 CSS。

       句子按稿子渲染成**均匀的一段灰字**：稿子里「12 个子体在未来 14 天内…」
       的数字和汉字同一字重。所以这里不再走后端给的 parts 逐段加重
       （那一层 09-01 做的，后端仍照旧输出 parts，想恢复就把 text 换回 ledeText）。 */
    const body = ctx.el('div', { class: 'ovw-lede-body' }, [
      ctx.el('p', { class: 'ovw-lede-text', text: ln.text || '' }),
      moduleLink(ctx, ln.module, '/' + ln.module, go ? go + ' ›' : '›'),
    ]);
