...
/* 导出。走 <a download> 而不是 fetch + blob —— 文件名由后端的
   Content-Disposition 决定，浏览器原生下载就用它，不必在前端重拼。 */
...
function overviewView(d) {
  ...
  return el("div", { class: "cmp-body" }, [
    el("section", { class: "cmp-sec" }, [
      el("div", { class: "cmp-sec-h" }, [
        el("span", {}, [`威胁比较表 · ${d.totals.shown} / ${d.totals.analyzed} 个竞品产品族`,
              ctx.aiMark("竞品威胁判定"),
              ctx.schemePicker("竞品威胁判定")]),
            el("button", { class: "wb-export", type: "button", title: "导出成 Excel（.xlsx），可直接给其他部门下单用",
        onclick: (e) => downloadExport(e.currentTarget, "/api/competitor/export") }, "导出 Excel"),
